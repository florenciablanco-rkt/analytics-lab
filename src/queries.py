...
def q_funnel(cfg: ClientConfig, start: date, end: date,
             app_ids: list[str] | None = None) -> str:
    """Funnel de suscripción por semana × país × canal: installs, suscriptores
    mensuales/anuales, y add-ons con revenue real. Base del producto (ViX)."""
    apps = app_ids or cfg.app_ids
    inst, buy = cfg.install_event, cfg.purchase_event
    sub = cfg.subscription
    subm = sub.get("monthly_event", "cdp_subscribe_monthly")
    suba = sub.get("annual_event", "cdp_subscribe_annual")
    markets = list(sub.get("prices", {}).keys())
    country_filter = f"    AND country IN ({_in_list(markets)})\n" if markets else ""
    return f"""SELECT
    date_trunc('week', CAST(substr(dt, 1, 10) AS DATE))                              AS semana,
    country,
    COALESCE(partner, pid)                                                           AS canal,
    COUNT(DISTINCT CASE WHEN event_name = '{inst}' THEN mmp_device_id END)           AS installs,
    COUNT(DISTINCT CASE WHEN event_name = '{subm}' THEN mmp_device_id END)           AS sub_monthly,
    COUNT(DISTINCT CASE WHEN event_name = '{suba}' THEN mmp_device_id END)           AS sub_annual,
    COUNT(DISTINCT CASE WHEN event_name = '{buy}'  THEN mmp_device_id END)           AS add_on_buyers,
    SUM(           CASE WHEN event_name = '{buy}'  THEN event_revenue_usd END)        AS add_on_revenue
FROM prod_tracking.postbacks_typed
WHERE app_id IN ({_in_list(apps)})
    AND event_name IN ('{inst}', '{subm}', '{suba}', '{buy}')
{country_filter}{_date_filter(start, end)}
GROUP BY 1, 2, 3
ORDER BY semana, country, installs DESC
"""
